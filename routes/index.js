    ...
    try {
        ...
        if (user) {
            ...
            const [rows] = await db.query(`
                SELECT p.*, u.userId AS ownerName FROM products p JOIN users u ON p.userId = u.id
                WHERE p.userId IN (SELECT friendId FROM friends WHERE userId = ?) ORDER BY p.id DESC LIMIT 3
            `, [user.id]);
            friendProducts = rows;
...
router.get('/chat', async (req, res) => {
    ...
    const [chatUsers] = await db.query(`
        SELECT DISTINCT u.id, u.userId FROM users u JOIN messages m ON (u.id = m.senderId OR u.id = m.receiverId)
...
router.get('/chat/:sellerId', async (req, res) => {
    ...
    const [messages] = await db.query(`
        SELECT m.*, u.userId as senderName FROM messages m JOIN users u ON m.senderId = u.id
        WHERE (m.senderId = ? AND m.receiverId = ?) OR (m.senderId = ? AND m.receiverId = ?) ORDER BY m.createdAt ASC
    `, [req.session.user.id, sellerId, sellerId, req.session.user.id]);
